...
class DatasetFSS(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, test_num=1000):
        ...
        self.class_names = ['ac wall', 'acorn', 'adhensive tape', 'adidas logo1', 'adidas logo2', 'afghan hound',
                            ...
                            'eggnog', 'egret', 'egyptian cat', 'electric fan', 'electronic toothbrush',
                            'eletrical switch',
                            ...
                            'raft', 'rain barrel', 'recreational vehicle', 'red bayberry', 'red breasted merganser',
                            'red wolf',
                            'redheart', 'remote control', 'rhinoceros', 'ringlet butterfly', 'rock beauty', 'rocket',
                            'roller coaster', 'roller skate', 'rosehip', 'ruddy turnstone', 'ruffed grouse',
                            'running shoe',
                            ...
                            'espresso maker', 'fire balloon', 'fire hydrant', 'fire screen', 'flat-coated retriever',
                            'folding chair',
                            ...
                            'american alligator', 'american chamelon', 'andean condor', 'anise', 'apple icon',
                            'arch bridge',
                            'arrow', 'astronaut', 'australian terrier', 'bamboo dragonfly', 'bamboo slip',
                            'banana boat',
                            ...
                            'cantilever bridge', 'canton tower', 'captain america shield', 'carriage',
                            'cathedrale paris', 'cd',
                            ...
                            'sydney opera house', 'taj mahal', 'tiltrotor', 'toaster', 'tower pisa',
    ...
    def __getitem__(self, idx):
        ...
        query_image, query_cmask, support_images, support_cmasks = self.load_frame(query_name, support_names)
    ...
    def load_frame(self, query_name, support_names):
        ...
        support_names = [os.path.join(os.path.dirname(name), sid) + '.png' for name, sid in
                         zip(support_names, support_ids)]
    ...
    def sample_episode(self, idx):
        ...
        while True:
            support_num = np.random.choice(range(1, 11), 1, replace=False)[0]
            support_name = os.path.join(os.path.dirname(query_name), str(support_num)) + '.jpg'
