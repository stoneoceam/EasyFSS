...
class Visualizer:
    ...
    @classmethod
    def initialize(cls, visualize, args):
        ...
        if args.visual_fold_name == '':
            ...
            cls.vis_path = os.path.join('vis', args.visual_fold_name)
    ...
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b,
        ...
        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, query_name, support_name) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, query_names,
                              support_names)):
    ...
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx,
        ...
        merged_pil = cls.merge_image_pair(
            [spport_image, query_image] + spt_masked_pils + [qry_masked_pil, pred_masked_pil])
        ...
        merged_pil.save(os.path.join(cls.vis_path,
                                     '%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '.jpg'))  # fss
